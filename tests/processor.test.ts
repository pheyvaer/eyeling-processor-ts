...
describe("Eyeling processor tests", async () => {
  test("rdfc:EyelingProcessorTs is properly defined", async () => {
    ...
    const config = helper.getConfig(
      new NamedNode("https://eyereasoner.github.io/eyeling#EyelingProcessorTs"),
    );
    ...
    expect(proc.input.constructor.name).toBe("Array");
    expect(proc.rules.constructor.name).toBe("ReaderInstance");
